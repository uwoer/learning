...
import com.uwola.learning.scan.{ActionsUpdate, PagesUpdate, TestScanner, VisitsUpdate}
...
//      .appName("Spark Hbase Example")
      .appName("Spark Hbase Data Completion APP")
...
//      .config("spark.executor.extraJavaOptions", "-Xdebug -Xrunjdwp:transport=dt_socket,address=10001,server=y,suspend=n")
      .getOrCreate()
//    TestScanner.scanner(spark.sparkContext,spark.sqlContext)
//    TestPut.put(spark.sparkContext,spark.sqlContext)
//    VisitsUpdate.update(spark)
    PagesUpdate.update(spark, start, end)
//    ActionsUpdate.update(spark, start, end)
